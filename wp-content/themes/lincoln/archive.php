				...
				<?php
					...
					if ( have_posts() && ! isset( $ld_pt ) ) :
						...
						if ( is_plugin_active( 'sfwd-lms/sfwd_lms.php') && isset( $ld_pt ) && !empty( $ld_pt ) ) :
							...
							if ( isset( $smof_data['ld-archive-limit'] ) && $smof_data['ld-archive-limit'] ) {
								$limit = $smof_data['ld-archive-limit'];
							} 
							$paged = max( 1, $_GET['ld-paged'] );
							switch ( $ld_pt ) {
								case 'sfwd-courses':
									if ( is_tag() ) :
										$tax = 'tag';
					...
					if ( isset( $ld_pt ) ) {
						include_once K2T_TEMPLATE_PATH . 'navigation-learndash.php';
					} else {
