...
$ld_hidden = '';
if ( isset( $ld_pt ) ) {
	$ld_hidden = isset( $smof_data['ld-archive-date'] ) &&  ! $smof_data['ld-archive-date'] ? ' ld-hidden-date' : '';
	$ld_hidden .= isset( $smof_data['ld-archive-cat'] ) &&  ! $smof_data['ld-archive-cat'] ? ' ld-hidden-cat' : '';
	$ld_hidden .= isset( $smof_data['ld-archive-link'] ) &&  ! $smof_data['ld-archive-link'] ? ' ld-hidden-link' : '';
}

?>

<article id="post-<?php the_ID(); ?>" <?php post_class( 'element hentry post-item ' . $post_format . '-post' ); ?>>
	<div class="post-inner k2t-element-hover <?php echo $ld_hidden;?>">
		...
		<?php if ( 'quote' == $post_format ) : ?>
			<header>
				...
				<?php if ( $smof_data['blog-author'] ) : ?>
					<span class="entry-author"><i class="zmdi zmdi-account"></i><a class="" href="<?php echo esc_url($author_link); ?>"><?php the_author(); ?></a></span>
				<?php endif;?>
		...
		<?php if ( 'quote' != $post_format ) : ?>
			...
			<div class="entry-content clearfix">
				...
				<header>
					...
					<?php if ( $smof_data['blog-author'] ) : ?>
						<?php 
						if ( isset( $ld_pt ) && !empty( $ld_pt ) ) :
							$author_link = add_query_arg( 'ld-pt', $ld_pt , get_author_posts_url( get_the_author_meta( 'ID' ) ) );
						else :
							$author_link = get_author_posts_url( get_the_author_meta( 'ID' ) );
						endif;
						?>
						<span class="entry-author"><i class="zmdi zmdi-account"></i></i><a class="" href="<?php echo esc_url($author_link); ?>"><?php the_author(); ?></a></span>
					<?php endif;?>
		...
		<?php if ( 'quote' == $post_format ) : ?>
			...
			<?php if( isset( $smof_data['blog-categories-icons'] ) && $smof_data['blog-categories-icons'] == '1' ): ?>
				<span title="<?php echo get_cat_name($post_categories[0]); ?>" class="cat-icon" style = "background-color : <?php function_exists( 'get_field' ) ? the_field('category_color', 'category_' . $post_categories[0]) : '';?>">
					<?php function_exists( 'get_field' ) ? the_field('category_icon', 'category_' . $post_categories[0]) : '<i class="zmdi zmdi-folder-outline"></i>';?>
				</span>
			<?php endif; ?>
			<div class= "footer-content clearfix">
				<?php 
