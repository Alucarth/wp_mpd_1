	    ...
	    <?php if ( 'quote' != $post_format ) : ?>
		    ...
		    <div class="entry-content clearfix">
		    	<?php if( isset( $smof_data['blog-categories-icons'] ) && $smof_data['blog-categories-icons'] == '1' ): ?>
			    	<a href="<?php echo get_category_link($post_categories[0]);?>" title="<?php echo get_cat_name($post_categories[0]); ?>" class="cat-icon" style = "<?php echo ( function_exists( 'get_field' ) ) ? 'background-color: ' . get_field('category_color', 'category_' . $post_categories[0]) . ';' : '' ;?>">
						<?php echo ( function_exists( 'get_field' ) ) ? the_field('category_icon', 'category_' . $post_categories[0]) : '<i class="zmdi zmdi-graduation-cap"></i>' ;?>
					</a>
				<?php endif; ?>
		    	<header>
		        ...
		        <?php
					if ( 'excerpts' == $smof_data['blog-display'] ) {
						...
						the_content( sprintf( esc_html('Readmore','k2t') ) );		
					}
				?>
