...
if ( !function_exists( 'lc_add_filter') ) {

	add_action( 'wp_head', 'lc_add_filter' );
	function lc_add_filter() {
		if ( !is_page() ) {
...
if( ! is_admin() ){
	...
	if ( ! function_exists( 'k2t_front_end_enqueue_script' ) ) :
		function k2t_front_end_enqueue_script() {
			...
			if ( isset( $smof_data['mm_sticky'] ) && $smof_data['mm_sticky'] ) {
				$mainParams['mm_sticky'] = $smof_data['mm_sticky'];
			}
		...
		function k2t_front_end_enqueue_inline_css() {
			...
			<style>
				...
				.learndash .wpProQuiz_button:hover {
					background: <?php echo $smof_data['primary-color'];?> !important;
				}
				.k2t-gallery-shortcode .view .mask {
				 	background: <?php 
				 		$rgb      			= k2t_hex2rgb( $smof_data['primary-color'] );
				 		echo 'rgba(' . $rgb['0'] . ',' . $rgb['1'] . ',' . $rgb['2'] . ',' . 0.9 .');';
				 	?>
				}
				...
				if ( isset( $smof_data['top-section-text-color'] ) && $smof_data['top-section-text-color'] ) {
					echo '.top-main-menu-mobile,' . 
						 '.k2t-header .k2t-header-m .top-main-menu-mobile .top-mm-right .shop-cart .cart-control i {' . 
							'color: ' . $smof_data['top-section-text-color'] . ';' . 
						 '}';
				}
	...
	if ( ! function_exists( 'k2t_enqueue_google_fonts' ) ) {
		function k2t_enqueue_google_fonts() {
			...
			wp_enqueue_style( 'k2t-main-font-roboto', 'https://fonts.googleapis.com/css?family=Roboto:300,300i,400,400i,500,700' );
			...
			function k2t_vc_map_shortcodes() {
				...
				$k2t_icon_box = array(
					...
					'params'          => array(
						...
						array(
							'param_name' => 'target',
							'heading' 	 => __( 'Open link in', 'k2t' ),
							'type' 		 => 'dropdown',
							'value'      => array(
								__( 'self', 'k2t' )    => '_self',
								__( 'new tab', 'k2t' )  => '_blank',
							),
						),
						array(
							'param_name'  => 'fontsize',
						...
						array(
							...
							'heading'     => __( 'Title hover color', 'k2t' ),
							'description' => 'only with title has link',
							'type'        => 'colorpicker',
						),
						array(
				...
				if ( is_plugin_active( 'sfwd-lms/sfwd_lms.php') ) {
					...
					$ld_courses = array(
						'base'            => 'ld_course_list',
						'name'            => __( 'LearnDash course list', 'k2t' ),
						'icon'            => 'fa fa-sliders',
						'category'        => __( 'LearnDash', 'k2t' ),
						'params'          => array(
							array(
								'param_name' => 'col',
								'heading' 	 => __( 'Animation', 'k2t' ),
							...
							array(
								'param_name'  => 'num',
								'heading'     => __( 'Limit course', 'k2t' ),
								'description' => __( 'Numeric only', 'k2t' ),
								'type'        => 'textfield',
								'holder'      => 'div',
								'value'		  => '-1'
							),
							array(
			                    'type' => 'autocomplete',
			                    'heading' => __( 'Select Course Categories', 'js_composer' ),
			                    'param_name' => 'cat',
			                    'description' => __( 'Input category name to search, empty category not include', 'js_composer' ),
			                    // There is autocomplete custom settings:
			                    'settings' => array(
			                        'values' => get_category_leanrdash_list(),
			                        'multiple' => true
			                    )
			                ),
							array(
				...
				/*  [ Embed ]
				- - - - - - - - - - - - - - - - - - - */
				if ( preg_match( '/lunartheme/', $_SERVER['SERVER_NAME'] ) || true ) {
					$k2t_fake = array(
						'base'            => 'fake_instagram',
						'name'            => __( 'Fake instagram', 'k2t' ),
						'icon'            => 'fa fa-terminal',
						'category'        => __( 'Media', 'k2t' ),
						'content_element' => true,
						'params'          => array(
					...
					$k2t_event_listing = array(
						...
						'params'          => array(
							...
							array(
								...
								'description' => __( 'Input category name to search', 'js_composer' ),
								'settings'    => array(
									'values'   => k2t_get_category_list( 'k-event-category' ),
									'multiple' => true,
			                    ),
			                    'dependency'  => array(
									'element' => 'style',
							...
							array(
								...
								'heading'     => __( 'Type of Event', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('All', 'k2t') 			  => 'all',
									__('Upcomming Events', 'k2t') => 'upcomming',
									__('Ongoing Events'  , 'k2t') => 'on_going',
								),
								'holder'      => 'div',
								'dependency'  => array(
									'element' => 'style',
							...
							array(
								'param_name'  => 'event_time_format',
								'heading'     => __( 'Event Time format', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('24h', 'k2t') => 'H:i',
									__('12h lowercase', 'k2t') => 'g:i a',
									__('12h uppercase', 'k2t') => 'g:i A',
								),
								'holder'      => 'div',
							),
							array(
							...
							array(
								...
								'heading'     => __( 'Show/Hide Teacher', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('Hide', 'k2t') => 'hide',
									__('Show', 'k2t') => 'show',
					...
					$k2t_course_listing = array(
						...
						'params'          => array(
							...
							array(
								...
								'description' => __( 'Input category name to search', 'js_composer' ),
								'settings'    => array(
									'values'   => k2t_get_category_list( 'k-course-category' ),
									'multiple' => true,
			                    )
			                ),
					...
					$k2t_teacher_listing = array(
						...
						'params'          => array(
							...
							array(
								...
								'heading'     => __( 'Enable as slider', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('No' , 'k2t') => 'no',
									__('Yes', 'k2t') => 'yes',
							...
							array(
								...
								'heading'     => __( 'Slider Item on Desktop', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('1 Columns', 'k2t') => '1',
									__('2 Columns', 'k2t') => '2',
							...
							array(
								...
								'heading'     => __( 'Slider Item on Tablet', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('1 Columns', 'k2t') => '1',
									__('2 Columns', 'k2t') => '2',
							...
							array(
								'param_name'  => 'navigation',
								'heading'     => __( 'Slider Navigation', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('Display', 'k2t') => 'show',
									__('Hide', 'k2t') 	 => 'hide',
							...
							array(
								...
								'description' => __( 'Input category name to search', 'js_composer' ),
								'settings'    => array(
									'values'   => k2t_get_category_list( 'k-teacher-category' ),
									'multiple' => true,
			                    )
			                ),

			                array(
								'param_name'  => 'filter_bar',
								'heading'     => __( 'Show/Hide Filter bar', 'k2t' ),
								'type'        => 'dropdown',
								'value'      => array(
									__('No' , 'k2t') => 'no',
									__('Yes', 'k2t') => 'yes',
					...
					$k2t_gallery_listing = array(
						...
						'params'          => array(
							...
							array(
								...
								'description' => __( 'Input category name to search', 'js_composer' ),
								'settings'    => array(
									'values'   => k2t_get_category_list( 'gallery-category' ),
									'multiple' => true,
			                    ),
			                ),
					...
					$k2t_project_listing = array(
						...
						'params'          => array(
							...
							array(
								...
								'description' => __( 'Input category name to search', 'js_composer' ),
								'settings'    => array(
									'values'   => k2t_get_category_list( 'k-project-category' ),
									'multiple' => true,
			                    ),
			                ),
							array(
...
add_filter( 'ld_course_list', 'k2t_learn_dash_course_lish', 10, 3 ); 

function k2t_learn_dash_course_lish( $output, $shortcode_atts, $filter ) {
	global $smof_data;
	$hidden_date = isset( $smof_data['sfwd-course-list-date'] ) 	? $smof_data['sfwd-course-list-date'] 		: '';
	$hidden_link = isset( $smof_data['sfwd-course-list-readmore'] ) ? $smof_data['sfwd-course-list-readmore'] 	: '';
	$classes  = 'b-masonry';
	$classes .= ! $hidden_date ? ' hidden-date' : '';
	$classes .= ! $hidden_link ? ' hidden-link' : '';
	$column   = $shortcode_atts['col'] ? $shortcode_atts['col'] : '2';
	$output = '<div class="' . $classes . '"><div class="masonry-layout column-' . $column . '">' . $output . '</div>' . '</div>';
	if ( $shortcode_atts['col'] == 1 ) {
		$output = str_replace( '600x340', '435x435', $output );
	}
	return $output;
}


function get_category_leanrdash_list() {
    ...
    if ( is_array( $terms ) && count( $terms ) ) :
        foreach ($terms as $key => $term) {
            ...
            if ( $query->posts ) {
                $listing[ $k ] = array();
				$listing[ $k ]['value'] = $term->term_id;
				$listing[ $k ]['label'] = $term->name;
				$k++;
            }
            wp_reset_postdata();
        }
...
if ( ! function_exists( 'k2t_get_category_list' ) ) {
	function k2t_get_category_list( $term_name = '' ) {

		$listing = array();

		$terms = get_terms( array( 'taxonomy' => $term_name, 'hide_empty' => false ) );
...
function k2t_lincoln_defer_parsing_of_js ( $url ) {
	if ( FALSE === strpos( $url, '.js' ) ) return $url;
	if ( strpos( $url, 'jquery.js' ) ) return $url;
	return "$url' defer ";
}
if ( ! is_admin() ) {
	// add_filter( 'clean_url', 'k2t_lincoln_defer_parsing_of_js', 11, 1 );
}
