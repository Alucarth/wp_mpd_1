...
if ( ! function_exists( 'k2t_setup' ) ) {
	...
	function k2t_setup() {
		...
		add_image_size( 'thumb_m_414x414', 414, 414, true );
	...
	function k2t_register_theme_dependency() {
		...
		$plugins = array(
			...
			array(
				...
				'slug'     => 'z-import-demo',
				'required' => false,
				'source'   => K2T_FRAMEWORK_PATH . 'extensions/plugins/z-import-demo.zip',
			),
		);
		...
		if ( is_front_page() ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( isset( $cat ) ) $cats = get_category_parents( $cat, TRUE, $delimiter );
...
if ( ! function_exists( 'k2t_social' ) ) {
	function k2t_social( $id_col, $id_element, $section ) {
		...
		$custom_color = $custom_color_hover = '';
		$custom_color =  k2t_decode( $data['columns'][$id_col]['value'][$id_element]['value']['color'] );
		$custom_color_hover =  k2t_decode( $data['columns'][$id_col]['value'][$id_element]['value']['color_hover'] );

		$header_section = '';

		switch ( $section ) {
			case 'header_section_1':
				$header_section = '.k2t-header-top';
				break;
			case 'header_section_2':
				$header_section = '.k2t-header-mid';
				break;
			case 'header_section_3':
				$header_section = '.k2t-header-bot';
				break;
		}

		if ( $custom_color || $custom_color_hover ) {
			$css  = '<style>';
			$css .= $header_section . ' .social li a { color: ' . $custom_color . ' !important;}';
			$css .= $header_section . ' .social li a:hover i { color: ' . $custom_color_hover . ' !important;}';
			$css .= '</style>';
			echo $css;
		}
...
if ( ! function_exists( 'k2t_logo' ) ) {
	function k2t_logo( $id_col, $id_element, $section ) {
		...
		?>
		<?php 
			$link = $smof_data['link_homepage'] ? $smof_data['link_homepage'] : esc_url( home_url( "/" ) ); 
			if ( is_front_page() ) $link = '#';
		?>
		<a class="k2t-logo" rel="home" href="<?php echo esc_url( $link );?>">
			<?php
...
if ( ! function_exists( 'k2t_login' ) ) {
	function k2t_login( $id_col, $id_element, $section ) {
		...
		$url  = k2t_decode( $data['columns'][$id_col]['value'][$id_element]['value']['custom_url'] ) ;
		$login_url  = k2t_decode( $data['columns'][$id_col]['value'][$id_element]['value']['custom_login_url'] ) ;
		$reg_url  = k2t_decode( $data['columns'][$id_col]['value'][$id_element]['value']['custom_register_url'] ) ;


		if ( is_user_logged_in() ) {
			$current_user = wp_get_current_user();
			$url  	   = $url ? $url : k2t_get_author_posts_url( $current_user->ID );
		} else {

			$login_url = isset( $login_url ) && $login_url != '' ? $login_url : wp_login_url( get_permalink() ) ;
			$reg_url   = isset( $reg_url ) && $reg_url != '' ? $reg_url : wp_registration_url() ;
		}
		...
		if ( is_user_logged_in() ) :
        	echo __('Hello','k2t') . ' <a href="' . esc_url( $url )  . '">' . $current_user->user_login . '! ';
        	echo '<a href="' . wp_logout_url( get_home_url() ) . '"> '. __( 'Logout', 'k2t') . '</a>  <i class="zmdi zmdi-square-right"></i>';
		else: 
			echo '<a href="'. esc_url( $login_url ) .'" title="'. __( 'Login', 'k2t' ) .'">'. __( 'Login', 'k2t' ) .'</a>  /  <a href="' . $reg_url .'" title="'. __( 'Sign Up', 'k2t' ) .'">'. __( 'Sign Up', 'k2t' ) .'</a>';
		endif;
		echo '</div>';
		return;
	}
 ...
 */
function k2t_get_author_posts_url( $author_id, $author_nicename = '' ) {
	global $wp_rewrite;
