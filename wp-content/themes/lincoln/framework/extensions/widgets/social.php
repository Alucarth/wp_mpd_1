...
class k2t_Widget_Social extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$url_wp = isset ( $instance['wp_link'] ) ? $instance['wp_link'] : '';
		echo ( $before_widget );
		$social_html = '';
		...
		if(!empty($url_wp)) {
			// $url_vk = 'https://' . str_replace( 'https://', '', $url_vk );
			$social_html .= "<li class='wp'><a target='_blank' href='https://api.whatsapp.com/send?phone=$url_wp&text=Hola Plan Generación de Empleo, tengo una consulta.'><i class='fa fa-whatsapp'></i></a></li>";
		}
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['wp_link'] = $new_instance['wp_link'];
		return $instance;
	}
...
?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'wp_link' ) ); ?>"><?php _e( 'Whatsapp:', 'k2t' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'wp_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'wp_link' ) ); ?>" type="text" value="<?php echo esc_attr( $wp_link ); ?>" />
		</p>
