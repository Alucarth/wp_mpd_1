            ...
            <div class="row slider-item">        	
                		...
                		<% if (events.length < 2) { %>
                            <span class="number-event-pd">
                            	<%if(event.posttype=="u_course") {%>
                                	<?php __("Course",'k2t-event');?> <%=i%>/<%=events.length%>
                                <% } else {%>
    								<?php __("Event",'k2t-event');?> <%=i%>/<%=events.length%>
                                <%}%>
    						</span> 
                        <% } else { %>
                ...
                <?php echo '<%if (event.picture !=null) {%>'?>
                    ...
                    <div class="col-md-9 col-sm-9">                
                        ...
                        <span class="stm-time"><?php echo __("Time: ",'k2t-event');?> <%if(event.posttype!="u_course") {%> <%= event.startDate %> / <%}%> <%= event.endDate %></span>
                        <span class="stm-location"><?php echo __("Location: ",'k2t-event');?><%= event.location %></span>
                        <a href="<%= event.url ? event.url : 'javascript:void(0)' %>" class="event-btt"><?php echo __("BUY TICKET",'k2t-event');?><%= event.buyticket %></a>
                    </div> 
                <?php echo " <%}else{%> "?>
