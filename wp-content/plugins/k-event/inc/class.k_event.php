...
Class K_Event {
	static function K_Render_event_listing_calendar ( $style ) {
		...
		$calendar_params['m0'] = __('January','k2t-event');
		$calendar_params['m1'] = __('February','k2t-event');
		$calendar_params['m2'] = __('March','k2t-event');
		$calendar_params['m3'] = __('April','k2t-event');
		$calendar_params['m4'] = __('May','k2t-event');
		$calendar_params['m5'] = __('June','k2t-event');
		$calendar_params['m6'] = __('July','k2t-event');
		$calendar_params['m7'] = __('August','k2t-event');
		$calendar_params['m8'] = __('September','k2t-event');
		$calendar_params['m9'] = __('October','k2t-event');
		$calendar_params['m10'] = __('November','k2t-event');
		$calendar_params['m11'] = __('December','k2t-event');

		$calendar_params['ms0'] = __('Jan','k2t-event');
		$calendar_params['ms1'] = __('Feb','k2t-event');
		$calendar_params['ms2'] = __('Mar','k2t-event');
		$calendar_params['ms3'] = __('Apr','k2t-event');
		$calendar_params['ms4'] = __('May','k2t-event');
		$calendar_params['ms5'] = __('Jun','k2t-event');
		$calendar_params['ms6'] = __('Jul','k2t-event');
		$calendar_params['ms7'] = __('Aug','k2t-event');
		$calendar_params['ms8'] = __('Sep','k2t-event');
		$calendar_params['ms9'] = __('Oct','k2t-event');
		$calendar_params['ms10'] = __('Nov','k2t-event');
		$calendar_params['ms11'] = __('Dec','k2t-event');

		$calendar_params['d0'] = __('Sun','k2t-event');
		$calendar_params['d1'] = __('Mon','k2t-event');
		$calendar_params['d2'] = __('Tue','k2t-event');
		$calendar_params['d3'] = __('Wed','k2t-event');
		$calendar_params['d4'] = __('Thu','k2t-event');
		$calendar_params['d5'] = __('Fri','k2t-event');
		$calendar_params['d6'] = __('Sat','k2t-event');

		$calendar_params['buy_text'] = __('BUY TICKET ','k2t-event');
		$calendar_params['st_text'] = __('Start: ','k2t-event');
		$calendar_params['en_text'] = __('End: ','k2t-event');
		$calendar_params['loca_text'] = __('Location: ','k2t-event');
	...
	static function K_Render_event_listing_default ( $post_per_page, $event_pagination, $arr_term_id = '', $event_type = '', $show_excerpt, $show_teacher, $taxonomy = 'k-event-category' ) {
		...
		$class_ex = ( $show_excerpt ) ? 'show-excerpt' : '';
		$class_ex .= ( $show_teacher) ? ' show-teacer' : '';
		...
		} elseif( $event_type == 'on_going' ) {
			$arr['order'] 		= 'ASC';
			$arr['meta_key'] 	= 'event_end_date';
			$arr['meta_query']	= 	array(
										'relation' 			=> 'AND',
										array(
											'key'	 	=> 'event_start_date',
		...
		?>
		<div class="event-listing-classic <?php echo $class_ex;?>">
			<?php
			...
			?>
				<article class="event-classic-item k2t-element-hover">
					<div class="left-bg" style="background-image: url( <?php echo $post_thumb_url;?> )"></div>
					...
					<?php 
						$teacher = function_exists('get_field') ? get_field( 'event_teacher', get_the_ID() ) : '';
						if ( is_array( $teacher ) && count( $teacher ) > 0 ) : ?>
						<div class="entry-speakers">
							<h3 class="h"><i class="zmdi zmdi-mic"></i><span><?php _e( 'Speakers', 'k2t-event' )?></span></h3>
							<div class="k2t-row">
					...
					<div class="event-meta">
						<span class="date">
							...
							<time data-time="<?php echo esc_attr( date_i18n( 'Y-m-d\TH:i:s+00:00', $start_date ) ); ?>" class="entry-date"><?php echo esc_html( date_i18n( get_option('date_format') . ' ' . get_option('time_format') , $start_date ) ); ?></time>
							<?php endif;?>
						</span>
					...
					<a class="more-link btn-ripple" href="<?php echo esc_url( $post_link );?>" title="<?php echo esc_attr( $title );?>"><?php _e('Join', 'k2t-event');?><i class="zmdi zmdi-chevron-right"></i></a>
					</div>
					<!-- .wrap-left -->
				</article>
			<?php
	...
	static function K_Render_event_listing_masonry ( $masonry_column, $post_per_page, $event_pagination, $event_masonry_filter, $arr_term_id = '', $taxonomy = 'k-event-category', $event_time_format = 'H:i', $event_type = 'all' ) {
		...
		<div class="event-listing-masonry-wrapper">
			<?php if ( $event_masonry_filter == 'show' ): ?>
				<?php 
					...
					if ( count( $arr_term_id ) ) {
						$arr_obj_term = array();
						$arr_term_id = explode( ',', $arr_term_id );
						foreach ( $arr_term_id as $id ) {
							$arr_obj_term[] = get_term( $id, 'k-event-category' );
						}
						$categories = $arr_obj_term;
					}
				...
				?>
					<?php if(isMobile()) : ?>
						<select id="cd-dropdown" class="cd-select k2t-isotope-filter">
							<option value="-1" selected><?php _e( 'Sort Event', 'k2t-event' );?></option>
							<option class="*"><?php _e( 'All', 'k2t-event' );?></option>
					...
					<?php else: ?>
						<ul class="event-isotope-filter filter-list">
							<li class="*"><?php _e( 'All', 'k2t-event' );?></li>
							<?php foreach($categories as $category):?>
			...
			<div class="event-listing-masonry <?php echo esc_html($masonry_column); ?>">
				...
				if( count( $query->posts ) > 0 ):
					while( $query->have_posts() ) : $query->the_post();
						self::grid_render_content( $event_time_format, $masonry_column );
					endwhile;
				endif; ?>
	...
	static function K_Render_event_listing_carousel ( $masonry_column, $number_post_show, $event_pagination, $event_navigation, $arr_term_id = '', $event_type, $taxonomy = 'k-event-category' ) {
		$arr = array(
			'post_type'      => 'post-k-event',
	...
	static function grid_render_content ( $event_time_format = 'H:i', $masonry_column = '' ) {
		$event_address = (function_exists('get_field')) ? get_field('event_address', get_the_ID()) : ''; $event_address = empty($event_address) ? '' : $event_address;
		$thumbnail = (has_post_thumbnail(get_the_ID())) ? true : false;
		$categories = get_the_terms(get_the_ID(), 'k-event-category');
		$title = get_the_title();
		...
		$post_link = get_permalink(get_the_ID());
		$post_thumb_size = 'thumb_600x600';
		$masonry_column = str_replace( 'columns-', '', $masonry_column );
		if ( $masonry_column >= 3 && preg_match( '/lunartheme/', $_SERVER['SERVER_NAME'] ) ) {
			$post_thumb_size = 'thumb_m_414x414';
		}
		$post_thumb = '<a href="'. esc_url( $post_link ) .'" title="'. esc_attr( $title ) .'">' . get_the_post_thumbnail(get_the_ID(), $post_thumb_size, array('alt' => trim(get_the_title()))) . '</a>';
		$post_thumb_url = wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) );
		$start_date = ( function_exists('get_field')) ? get_field( 'event_start_date', get_the_ID() ) : '';
			...
			if( $the_query->have_posts() ) {
				...
				while( $the_query->have_posts() ){ 
					...
					$ar_rs = array(
						...
						'startDate'				=> date( get_option('time_format'), strtotime($start_date) ), //date_i18n( get_option('date_format'), strtotime($start_date)),
