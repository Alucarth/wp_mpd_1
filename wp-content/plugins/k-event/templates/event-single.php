						...
						<div class="k2t-row">
									...
									<div class="entry-meta-inner">
										<div class="product-price">
											<?php 
												...
												if ( !empty( $product ) && class_exists('WC_Product') ):
											?>
												<h4><?php _e( 'Price', 'k2t-event' )?></h4>
												<span class="price">
													<?php 
										...
										<?php if( isset( $show_hide_number_tickets) && $show_hide_number_tickets == 'show' ): ?>
											<div class="event-ticket">
												<h4><?php _e( 'Number of Tickets:', 'k2t-event' )?></h4>
										...
										<?php if( isset( $show_hide_button_join ) && $show_hide_button_join == 'show' ): ?>
											<?php $product = (function_exists('get_field')) ? get_field('event_product', get_the_ID()) : ''; $product = empty($product) ? '' : $product;
												if(!empty($product)):
													...
													$actual_link = preg_replace('/\?add-to-cart.*/', '');
													$actual_link .= '?add-to-cart=' . $product->ID . '&quantity=1';
											?>
												<a href="<?php echo $actual_link; ?>" target="_self" class="event-link add-to-cart-link btn-ripple" ><?php echo !empty($subscribe_button_text) ? $subscribe_button_text : __('Launch Event', 'k2t-event'); ?></a>
											<?php else :?>
					                            <a href="<?php echo esc_url($subscribe_url);?>" target="_self" class="event-link btn-ripple" ><?php echo !empty($subscribe_button_text) ? $subscribe_button_text : __('Launch Event', 'k2t-event'); ?></a>
					                        <?php endif;?>
				                    	<?php endif; ?>
							...
							<div class="entry-content col-8">
								...
								<div class="k2t-row">
									...
									<?php if ( ! empty( $start_date ) && ! empty( $end_date ) ) : $start_date = strtotime( $start_date ); $end_date = strtotime( $end_date ); ?>
										<div class="col-6 event-time">
											<h3 class="h"><i class="zmdi zmdi-time"></i><span><?php _e( 'Time', 'k2t-event' )?></span></h3>
											<?php if(!empty($start_date)) : ?>
												<div class="start-event">
													<span><?php _e( 'Start:', 'k2t-event' ); ?></span>
											...
											<?php if(!empty($end_date)) : ?>
												<div class="end-event">
													<span><?php _e( 'End:', 'k2t-event' ); ?></span>
									...
									<?php if ( ! empty( $address ) ) : ?>
										<div class="col-6 event-address">
											<h3 class="h"><i class="zmdi zmdi-pin"></i><span><?php _e( 'Address', 'k2t-event' )?></span></h3>
								...
								<div class="event-description">
									<h3 class="h"><i class="zmdi zmdi-dot-circle"></i><span><?php _e( 'Description', 'k2t-event' )?></span></h3>
									<div class="content">
										<?php the_content();?>
									</div>
								</div>
								...
								<?php if( isset( $show_hide_button_join ) && $show_hide_button_join == 'show' ): ?>
	                            	<a href="<?php echo esc_url($subscribe_url) ? esc_url($subscribe_url) : get_post_permalink( $product->ID ) ;?>" target="_self" class="event-link btn-ripple k2t-element-hover" ><?php echo !empty($subscribe_button_text) ? $subscribe_button_text : __('Launch Event', 'k2t-event'); ?></a>
