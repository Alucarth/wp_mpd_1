...
function k_gallery_textdomain_plugin(){
	load_plugin_textdomain( 'k2t-gallery', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
...
if ( !function_exists( 'k2t_include_gallery_single_template' ) ) {
	function k2t_include_gallery_single_template ( $single_template ) {
		global $post;
		if ( is_singular('post-gallery') ){
			$single_template = k_gallery_include_template( 'gallery-single.php', false );
		}
		return $single_template;
	}
	add_filter( 'single_template', 'k2t_include_gallery_single_template' );
}

// replace template 
function k_gallery_include_template( $template = '', $include = true ){
	$child_path = get_stylesheet_directory();
	$template_path = locate_template( array( 'k-gallery/' . $template ) );
	if ( ! $template_path ) {
...
require_once( dirname(__FILE__) . '/inc/acf-content.php' ); // Add metadata of gallery
