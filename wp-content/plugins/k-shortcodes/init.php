...
function k_shortcode_textdomain_plugin(){
	load_plugin_textdomain( 'k2t-shortcodes', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}

if ( ! function_exists( 'k2t_return_list_shortcode' ) ) {
	function k2t_return_list_shortcode() {
		$shortcodes = 'animation, google_map, heading, dropcap, button, pre, pullquote, sub, sup, tooltip, br, clear, spacer, countdown, iconlist, fullwidth, imagebox, box, iconbox, hr, highlight, k2t_embed, k2t_slider, member, pricing, pricing_column, progress, toggle, accordion, tab, container, content_slider, awesome_slider, icon, brands, testimonial, circle_button, align, res_text_element, section, blockquote, blog_post, register, k2t_grid_slider';

		$shortcodes .= ', fake_instagram';

		if ( preg_match( '/lunartheme/', $_SERVER['SERVER_NAME'] ) ) {
			$shortcodes .= ', fake_instagram';
		}
...
if ( class_exists( 'Vc_Manager' ) && ! function_exists( 'k2t_iconfont_settings_field' ) ) :
	...
	vc_add_shortcode_param( 'k2t_icon', 'k2t_iconfont_settings_field' );
endif;
...
if ( ! function_exists ( 'k2t_shortcodes_fontend_scripts' ) ) :
	function k2t_shortcodes_fontend_scripts() {
		...
		if ( ! is_admin() ) {
			...
			// Enqueue Script
			wp_register_script( 'k2t-google-map', $str, array('jquery'), '', false );
			...
			wp_enqueue_script( 'k2t-inview', plugin_dir_url( __FILE__ ) . 'assets/js/vendor/jquery.inview.min.js', array( 'jquery' ), '1.0', true );
...
add_action( 'wp_head', 'k_shortcodes_remove_wpautop' );

function k_shortcodes_remove_wpautop() {
	if ( is_page() ) {
